...
class OtherChargeController extends Controller
{
    ...
    /**
     * School years to populate the Create/Edit form dropdown.
     *
     * Pulls distinct school_year values from active student_assessments first
     * (these are real years with enrolled students), then unions in a ±1-year
     * calendar window so future academic years are also reachable.
     *
     * Sorted descending — most recent year first — because:
     *   1. Charges are almost always created for the current or next AY.
     *   2. Create.vue defaults to schoolYears[0], which will now be the
     *      most recently active year rather than an arbitrary future offset.
     *
     * WHY THIS REPLACES THE OLD HELPER:
     *   Old version: calendar arithmetic only → generated ['2025-2026','2026-2027','2027-2028']
     *   Create.vue defaults to schoolYears[1] = '2026-2027'.
     *   Seeded assessments are in '2025-2026'.
     *   Result: preview always returned 0 students because no assessments
     *   existed yet for the pre-selected future year.
     */
    private function availableSchoolYears(): array
    {
        ...
        // Calendar window so future AYs are always reachable even before
        // assessments are created for them.
        $current   = (int) date('Y');
        $generated = [];
        ...
        // Merge, deduplicate, sort descending (most recent first).
        $all = array_unique(array_merge($fromDb, $generated));
        rsort($all);
