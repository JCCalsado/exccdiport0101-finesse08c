...
import {
    ...
    CreditCard, RefreshCw, Hourglass, Landmark, X,
...
interface OtherCharge {
    ...
    // 'unpaid' | 'pending' | 'awaiting_confirmation' | 'awaiting_proof' | 'awaiting_approval' | 'paid' | 'failed' | 'cancelled'
    status: string;
    amount_paid: number;
    paid_at: string | null;
...
// ── Bank Transfer ──────────────────────────────────────────────────────────────
// OPTION D: student submits a reference number, then is redirected to the
// proof-upload page (Student/OtherCharges/ProofUpload.vue) to attach the receipt.
const showBankTransferModal = ref(false);
const bankTransferCharge    = ref<OtherCharge | null>(null);
const bankTransferForm      = useForm({ reference_number: '' });
const submittingBankTransfer = ref(false);

const openBankTransferModal = (charge: OtherCharge) => {
    bankTransferCharge.value = charge;
    bankTransferForm.reset();
    bankTransferForm.clearErrors();
    showBankTransferModal.value = true;
};

const closeBankTransferModal = () => {
    showBankTransferModal.value = false;
    bankTransferCharge.value    = null;
};

const submitBankTransfer = async () => {
    if (!bankTransferCharge.value) return;
    if (!bankTransferForm.reference_number.trim()) {
        bankTransferForm.errors.reference_number = 'Please enter your bank transfer reference number.';
        return;
    }
    ...
    try {
        ...
        const response = await fetch(route('student.other-charges.bank-transfer', bankTransferCharge.value.id), {
            method:      'POST',
            credentials: 'same-origin',
            headers: {
                'Content-Type':     'application/json',
            ...
            body: JSON.stringify({ reference_number: bankTransferForm.reference_number }),
        });
        ...
        router.visit(route('student.other-charges.proof.show', data.payment_id));

    } catch (err) {
        bankTransferForm.errors.reference_number = err instanceof Error
            ? err.message
            : 'Could not submit bank transfer. Please try again.';
    } finally {
        submittingBankTransfer.value = false;
    }
};

const goToProofUpload = (charge: OtherCharge) => {
    if (!charge.payment_id) return;
    router.visit(route('student.other-charges.proof.show', charge.payment_id));
};
...
const statusLabel = (status: string) => {
    const map: Record<string, string> = {
        ...
        awaiting_proof:         'Upload Proof Required',
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    >
                        ...
                        <div class="p-4 space-y-2">
                            ...
                            <div
                                v-if="charge.status === 'awaiting_proof' && charge.rejection_reason"
                                class="flex items-start gap-1.5 rounded-lg bg-red-50 border border-red-200 px-2.5 py-1.5 text-xs text-red-800"
                            >
                                <AlertTriangle class="h-3 w-3 flex-shrink-0 mt-0.5" />
                                <span>Your proof was rejected: {{ charge.rejection_reason }}</span>
                            </div>
                        ...
                        <div class="border-t border-gray-100 bg-gray-50 px-4 py-3">
                            ...
                            <!-- Bank transfer: proof needed -->
                            <div v-else-if="charge.status === 'awaiting_proof'" class="space-y-1.5">
                                <button
                                    @click="goToProofUpload(charge)"
                                    class="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 transition-colors"
                                >
                            ...
                            <!-- Other in-progress states (online pending, bank transfer awaiting approval) -->
                            <div
                            ...
                            <div v-else class="space-y-1.5">
                                ...
                                <button
                                    @click="openBankTransferModal(charge)"
                                    class="w-full inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors"
                                >
                                    <Landmark class="h-4 w-4" />
            ...
            >
                <div class="w-full max-w-md rounded-2xl bg-white shadow-2xl">
                    <div class="flex items-center justify-between border-b border-gray-200 p-5">
                        <h3 class="text-base font-bold text-gray-900">Pay via Bank Transfer</h3>
                        <button @click="closeBankTransferModal" class="rounded-lg p-1.5 hover:bg-gray-100 transition-colors">
                            <X class="h-4 w-4 text-gray-500" />
                        </button>
                    </div>

                    <div v-if="bankTransferCharge" class="p-5 space-y-4">
                        <div class="rounded-lg bg-gray-50 border border-gray-200 p-3 text-sm space-y-1">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium text-gray-900">{{ bankTransferCharge.title }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-muted-foreground">Amount</span>
                                <span class="font-bold text-gray-900">{{ formatCurrency(bankTransferCharge.amount) }}</span>
                            </div>
                        </div>

                        <p class="text-xs text-muted-foreground">
                            Transfer the exact amount to the school's bank account, then enter the
                            reference number from your bank's confirmation below. You'll upload the
                            receipt itself on the next page.
                        </p>
                        ...
                        <div>
                            ...
                            <input
                                v-model="bankTransferForm.reference_number"
                                type="text"
                                placeholder="e.g. BPI-2026-0012345"
                                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
                                :class="{ 'border-red-400': bankTransferForm.errors.reference_number }"
                            />
                            <p v-if="bankTransferForm.errors.reference_number" class="mt-1 text-xs text-red-600">
                                {{ bankTransferForm.errors.reference_number }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3 border-t border-gray-200 p-5">
                        <button
                            ...
                            :disabled="submittingBankTransfer"
                        ...
                        >
                            {{ submittingBankTransfer ? 'Submitting…' : 'Continue to Upload Proof' }}
                        </button>
                        <button
